...
def _load_transactions_from_excel(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        if 'amount' not in df.columns:
            logger_utils.warning(
                'Нет колонки "amount". Не сможем считать кешбэк и траты.'
            )
...
def generate_report_spending_by_category_json(
    ...
    if 'date' not in df.columns or 'amount' not in df.columns:
        logger_utils.error(
            'Для отчёта по категориям требуются колонки "date" и "amount".'
        )
        return {
            'report_type': 'spending_by_category',
            'client_id': client_id,
...
def generate_report_spending_by_weekday_json(
    ...
    if df.empty:
        return {
            ...
            'period': {
                ...
                'start_date': (
                    (reference_date or pd.Timestamp.now()) - pd.Timedelta(days=days)
                ).isoformat(),
            },
        }
    ...
    agg = spending.groupby('weekday')['amount'].sum().reindex(range(7), fill_value=0)
