...
def test_events_page_json():
    ...
    # Не требуем ключ "events", потому что его нет в текущей реализации
...
class TestMain:
    ...
    def temp_data_dir(self, tmp_path: Path):
        ...
        data_dir.mkdir()
        # Создаём файлы, которые реально требуются функциям отчётов
        (data_dir / "transactions.xlsx").touch()
        (data_dir / "user_settings.json").touch()

        # Подменяем базовый путь: теперь все относительные пути будут строиться от tmp_path
        original_cwd = Path.cwd()
        yield data_dir
        # Очистка: pytest сам удалит tmp_path, но можно явно удалить папку data
        if data_dir.exists():
    ...
    def test_main_flow_success(
        ...
        # Важно: мы НЕ мокаем Path. Мы полагаемся на реальную папку temp_data_dir
        # Но нам нужно, чтобы Path("data") указывал на temp_data_dir.
        # Самый простой способ — временно изменить рабочую директорию.
        import os
        ...
        try:
            ...
            # Проверки аргументов можно добавить при необходимости
        finally:
    ...
    def test_main_data_folder_missing(self, tmp_path: Path):
        ...
        try:
            # Делаем tmp_path текущей рабочей директорией
            os.chdir(tmp_path)
            ...
            with patch.object(main.logger_utils, "error") as mock_logger_error:
                ...
                # Ключевая проверка: в сообщении должно быть "Папка data не найдена"
                # Мы не проверяем точное совпадение всей строки, чтобы не зависеть от деталей форматирования
