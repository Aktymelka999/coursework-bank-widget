...
def test_load_transactions_valid(tmp_path):
    ...
    # Проверяем, что колонки переименованы внутри функции (если у тебя есть логика переименования)
    # Если нет — можно убрать эти проверки или подстроить под реальный код reports.py
...
def test_cashback_no_matching_currency(mock_settings, mock_load_excel):
    ...
    # Логика зависит от того, как у тебя реализована фильтрация по валюте.
    # Если валюта не совпадает — кэшбек может быть 0 или всё равно считаться.
